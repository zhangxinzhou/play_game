...
class QLearning:
    ...
    def predict(self, state):
        """ 根据输入的观察值,预测输出的动作值 """
        all_action = self.Q[state, :]
        max_action = np.max(all_action)
        max_action_list = np.where(all_action == max_action)[0]
    ...
    def save(self, npy_file='./model/qlearning_table.npy'):
...
class Agent:
    ...
    def train(self, max_episode):
        if args.train:
            for episode in range(max_episode):
                ...
                if episode % 20 == 0:
                    print("Episode %03s: stpes = %02s,reward=%0.1f" % (episode, ep_steps, ep_reward))
            self.model.save()
    ...
    def test_episode(self, render=False):
        ...
        print('test action is: ', action)


if __name__ == '__main__':
    # 使用gym创建宫环境,设置is_slippery为false降级环境难度
