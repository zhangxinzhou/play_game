...
class Sarsa:
    ...
    def predict(self, state):
        ...
        # 防止最大的Q值有多个,找出所有最大的Q,然后再随机选择
        # where函数返回一个array,每个元素为下标
    ...
    def save(self, npy_file='./model/sarsa_q_table.npy'):
...
class Agent:
    def __init__(self, env):
        ...
        self.gamma = 0.9
        self.e_greed = 0.1
        # type(self.state_dim)L <class 'tuple'>
        # type(self.observation_space.n): <class 'int'>
    ...
    def train(self, max_episode):
        if args.train:
            for episode in range(max_episode):
                ...
                if episode % 20 == 0:
                    print("Episode %30s: steps = %20s, reward = %.1f" % (episode, ep_steps, ep_reward))
            self.model.save()
